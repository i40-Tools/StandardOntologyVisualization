function loadArchitectureAlignment(architectureData) {
    ...
    for (var key in opts) {
        loadArchitectureAlignment[key] = getSet(key, loadArchitectureAlignment).bind(opts);
    }
    ...
    function refreshInput() {
        ...
        loadArchitectureAlignment[name](value);
        if (name == 'dataLength' || name == 'setLength') {
            if (name == 'setLength') {
                globalData = [] // we reshuffle everything
            }
            return refresh(architectureData)
        }
        refresh();
    }
    ...
    d3.selectAll('#inputs input')
        .each(function() {
            var sel = d3.select(this),
                name = sel.attr("name");
            sel.property("value", loadArchitectureAlignment[name]())
        })
    ...
    function refresh(data) {
        ...
        vennArea.selectAll('path.venn-area-path').transition()
            .duration(isFirstLayout ? 0 : loadArchitectureAlignment.duration())
            .attr('opacity', loadArchitectureAlignment.circleOpacity())
        ...
        vennArea.selectAll('circle.inner').data(function(d) {
            ...
            .duration(isFirstLayout ? 0 : loadArchitectureAlignment.duration())
            .attr('opacity', loadArchitectureAlignment.innerOpacity())
            .attr("cx", function(d) {
                return d.center.x
            })
            .attr("cy", function(d) {
        ...
        vennArea.exit().transition()
            .duration(loadArchitectureAlignment.duration())
            .attrTween('d', function(d) {
                return d.d
            })
            .remove()
        ...
        layout.packer().start()
        return loadArchitectureAlignment
    }
    return refresh(architectureData)
};
